...
from database.database import get_channelis
...
async def is_subscribed(filter, client, update):
    ...
    if user_id in ADMINS:
        return True
    member = await get_channelis()
    c = f"{FORCE_SUB_CHANNEL}"
    c2 = f"{FORCE_SUB_CHANNEL2}"
    c3 = f"{FORCE_SUB_CHANNEL3}"
    c4 = f"{FORCE_SUB_CHANNEL4}"
    try:
        member1 = member.get(c)        
        member2 = member.get(c)  
        member3 = member.get(c)   
        member4 = member.get(c)   
        if user_id in member1 and user_id in member2 and user_id in member3 and user_id in member4:
            print("Returning True")
            return True

    except Exception as e:
        print("In Passing Statement")
        pass
    try:
